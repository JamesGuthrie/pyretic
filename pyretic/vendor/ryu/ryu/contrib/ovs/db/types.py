...
IntegerType = AtomicType("integer", 0, (int, int))
RealType = AtomicType("real", 0.0, (int, int, float))
BooleanType = AtomicType("boolean", False, (bool,))
StringType = AtomicType("string", "", (str, str))
...
class BaseType(object):
    ...
    def __parse_uint(parser, name, default):
        value = parser.get_optional(name, [int, int])
        if value is None:
            value = default
        else:
            max_value = 2 ** 32 - 1
    ...
    def from_json(json):
        ...
        atomic_type = AtomicType.from_json(parser.get("type", [str, str]))
        ...
        if enum is not None:
            ...
            base.min = parser.get_optional("minInteger", [int, int])
            base.max = parser.get_optional("maxInteger", [int, int])
            ...
            base.min = parser.get_optional("minReal", [int, int, float])
            base.max = parser.get_optional("maxReal", [int, int, float])
            if (base.min is not None and base.max is not None
                    and base.min > base.max):
            ...
            if base.ref_table_name:
                base.ref_type = parser.get_optional("refType", [str, str],
                                                   "strong")
...
class Type(object):
    ...
    def from_json(json):
        ...
        parser = ovs.db.parser.Parser(json, "ovsdb type")
        key_json = parser.get("key", [dict, str, str])
        value_json = parser.get_optional("value", [dict, str, str])
        min_json = parser.get_optional("min", [int])
        max_json = parser.get_optional("max", [int, str, str])
        parser.finish()
